...
## 24. Go 客户端怎么提交位移（sarama / kafka-go）

> 前面 §22 的机制是通用的，但**具体 API 各语言差别很大**。Go 生态两个主流客户端的心智模型和 Java 很不一样。

### Kafka 概念 → Go API 对照

| Kafka 概念 | Java | **sarama**（IBM，最常用） | **kafka-go**（segmentio） |
|---|---|---|---|
| position（本地位置） | `consumer.position()` | `session.MarkOffset()` 标记 | 内部跟踪 |
| committed offset | `commitSync/Async()` | `session.Commit()` / 后台自动提交 | `CommitMessages()` / `CommitOffsets()` |
| 自动提交开关 | `enable.auto.commit` | `Consumer.Offsets.AutoCommit.Enable` | `Reader.CommitInterval`（0 = 手动） |
| 自动提交间隔 | `auto.commit.interval.ms` | `Consumer.Offsets.AutoCommit.Interval` | `Reader.CommitInterval` |
| `auto.offset.reset` | `auto.offset.reset` | `Consumer.Offsets.Initial`（`OffsetNewest` / `OffsetOldest`） | `Reader.StartOffset` |

### sarama 的关键心智模型：MarkOffset ≠ 已提交

```
session.MarkOffset(msg, "")   ← 只是【本地标记】，没有发送任何请求！
                                （可选也更新本地内存里的 offset 缓存）
        ↓ 然后靠两条路真正落地：
  ① 后台自动提交：AutoCommit.Interval（默认 1s）到了，批量提交已标记的 offset
  ② 显式提交：session.Commit() 立即提交已标记的 offset（阻塞、同步）
```

**很多人以为 `MarkOffset` 之后就安全了——不是。** 中间这段时间崩溃 / rebalance，标记就丢了 → 重复消费。

**所以"同步 vs 异步"在 sarama 里的对应物是**：

| 语义 | sarama 写法 |
|---|---|
| **异步**（不阻塞消费循环） | 只 `MarkOffset()`，交给后台 AutoCommit 定时批量提交 |
| **同步**（立即落地） | `MarkOffset()` 之后调 `session.Commit()` |

### kafka-go 的模型更直白

```go
for {
    msg, err := reader.FetchMessage(ctx)   // 拉一条
    // ... 处理 ...
...
`CommitMessages` 是**同步的**（等 broker 确认），`Reader.CommitInterval = 0`（默认）就是纯手动提交。

### Go 场景特有的三个坑

**① 消费循环里的 panic 会崩掉整个进程**（这点和 Java 完全不同）

```go
// Java：线程里抛异常只死那个线程，主进程照跑
// Go ：任何 goroutine panic 且没 recover → 【整个进程崩】
func (h *handler) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
    for msg := range claim.Messages() {
        // 如果这里的处理逻辑 panic 且没兜住
        process(msg)   // ← panic 了，整个服务跟着挂
        sess.MarkOffset(msg, "")
    }
    return nil
}
...
for msg := range claim.Messages() {
    func() {
        defer func() {
            if r := recover(); r != nil {
                log.Errorf("panic 处理消息 %v: %v", msg.Offset, r)
                // 决策：不 MarkOffset（让它重投）还是 Mark（跳过毒消息）
            }
        }()
        process(msg)
    }()
...
**这一点和后面第 7 题的沙箱是同源的**——Go 的 panic 不局限于单个 goroutine，所以在同一进程里跑"可能 panic 的代码"必须显式兜，或者干脆隔离到沙箱里。

**② rebalance 时未提交的 MarkOffset 会丢**

`MarkOffset` 只是本地状态。如果 rebalance 发生在"标记了但还没自动提交"的窗口里，这些标记直接失效 → 重复消费。**所以关键位置要用 `session.Commit()` 立即落地**，或者把 AutoCommit.Interval 调小（代价是提交请求变多）。

**③ 多 goroutine 处理同一 partition 会打乱提交顺序**

```go
// 反模式：起多个 goroutine 处理同一个 partition 的消息
for msg := range claim.Messages() {
    go process(msg)          // ← 处理完的顺序 ≠ offset 顺序
    sess.MarkOffset(msg, "") // ← 标记顺序也乱 → 可能标记了还没处理完的 offset
}
```

**一个 partition 内必须单 goroutine 顺序处理**（要并发就跨 partition 并发，或者用 §23 提到的 Pulsar 那种支持单条 ack 的系统）。Kafka 的 committed offset 是**一个数字**，天然只有"顺序/累积"语义。
...
func (h *handler) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
    for {
        ...
        case msg, ok := <-claim.Messages():
            if !ok {
                return nil // claim 被关闭（rebalance）
            }
            func() {
                defer func() { // ① 兜 panic，别让整个进程崩
                    if r := recover(); r != nil {
                        log.Errorf("panic: %v, offset=%d", r, msg.Offset)
                    }
                }()
                process(msg)
            }()
            sess.MarkOffset(msg, "") // ② 本地标记
            // ③ 关键位置显式提交（否则只靠后台定时批提交）
            if msg.Offset%1000 == 0 {
                if err := sess.Commit(); err != nil {
                    log.Warnf("commit 失败: %v", err) // rebalance 时会失败，正常
                }
            }
        case <-sess.Context().Done():
            return nil // ④ rebalance / 关闭信号，退出前可以再 Commit 一次兜底
        }
    }
}
```

**四个要点**：兜 panic、MarkOffset、关键点显式 Commit、监听 `sess.Context().Done()` 优雅退出。

---

# 四、2026.09.14 影石面试真题（平台/运维方向画像）

> 七道题里四道是运维/平台方向的（K8s 探针、Dockerfile 分层、Redis AOF、沙箱），三道后端/中间件向（覆盖索引、MySQL 优化、Kafka 位移提交）。
...
详见「三、MQ」的 **§21 Kafka 的四个位置**、**§22 位移提交**、**§23 Pulsar 对照**、**§24 Go 客户端提交位移**。

## 7. 为什么用沙箱

**先确认场景**：这类问题通常指"平台/Agent 要执行**不可信代码**"（大模型生成的代码、用户提交的代码、评测用例）。所以问题实质是：**为什么不能直接在本服务进程里跑？**

### 四类风险，进程内隔离一类都防不住

| 风险 | 进程内（goroutine + recover） | 沙箱（容器 / 微虚机） |
|---|---|---|
| **稳定性** | panic 崩整个进程；且 recover 也能被绕过（代码自己 recover / `runtime.Goexit` / unsafe 破坏运行时） | 崩在沙箱里，主服务无感 |
| **资源** | `for {}` 占满 P/M、疯狂分配内存直接 OOM 整个服务——**Go 限不住单个 goroutine 的 CPU** | cgroups 限 CPU/内存/磁盘/进程数，超了就杀 |
| **权限 / 逃逸** | 能 `os.Open("/etc/passwd")`、能 `http.Get` 打内网、能执行任意 syscall | 独立 rootfs + netns + seccomp + 非 root |
| **外部影响** | fork 炸弹、SSRF 打内网服务 | PID ns 挡 fork 炸弹，网络白名单挡 SSRF |

**一句话**：沙箱的目的是"**把不可信代码的爆炸半径限制在一个格子里**"——同时解决**稳定性、资源、权限**三件事。

### Go 特有的那条理由：panic 会崩整个进程

```
Java：线程里抛未捕获异常 → 只死那个线程，主进程不受影响
Go  ：任何 goroutine panic 且没 recover → 【整个进程崩】
```

所以在 Go 里"起个 goroutine 跑不可信代码"的代价格外大：**一个 panic 就把主服务带走了**。但要注意 **panic 只是最轻的一类失败**——更严重的是**资源耗尽和权限逃逸**，这两个用 recover 完全防不住。

**两个术语要准确**：

- **不是"fork 出一个协程"**：`fork` 是系统调用（复制整个进程），`goroutine` 是用户态协程；而且 **Go 运行时不能安全 fork**（多线程程序 fork 后只有调用线程存活，锁可能被留在上锁状态 → 死锁）。正确说法是"**起一个 goroutine**"。
- **不是"隔离上下文"**：Docker 隔离的是六个明确维度——**文件系统 / 网络 / 进程 / 用户 / 资源（cgroup）/ 系统调用（seccomp）**。
...
弱 ──────────────────────────────────────────────► 强
语言级              进程级            容器           微虚机
RestrictedPython   seccomp/nsjail    Docker        Firecracker / Kata
isolated-vm(vm2)   bubblewrap        (+gVisor)     (microVM)

↑ recover 根本不在这个光谱上——它不构成隔离
```

**容器共享宿主内核 → 内核漏洞就是逃逸路径**，所以有 gVisor（用户态重实现系统调用）和 Kata/Firecracker（微虚机，多一层硬件虚拟化）。

### Docker 沙箱的加固清单

```
非 root 运行（user namespace）
+ 只读 rootfs
+ drop 所有 capability
+ seccomp 白名单（过滤危险 syscall）
+ 禁用网络（或只允许白名单，防 SSRF 打内网）
+ cgroup 限额（CPU / 内存 / 磁盘 / 进程数）
+ 临时文件系统（tmpfs）
```
...
```
每个请求起一个新容器 → 几百 ms ~ 秒级 → Agent/评测场景完全不可接受
  解法：① 预热池（常驻一批沙箱复用）
        ② microVM 快照恢复（Firecracker 强项，几十 ms）
```

**"沙箱怎么做"的难点不在隔离，在"隔离的同时还要快"。**

**超时必须 kill 容器**（不是 kill 进程），还要防止僵尸沙箱。

### 面试话术

> "沙箱的目的是把**不可信代码**的爆炸半径限制在一个格子里。为什么不能在本服务进程里跑？四类风险进程内都防不住：
>
> **一是稳定性**——Go 里任何 goroutine panic 不 recover 会崩掉整个进程（这点和 Java 线程不同），而且即使 recover 也不可靠，代码能自己 recover、能 Goexit、能用 unsafe 破坏运行时；
> **二是资源**——死循环或疯狂分配内存就能拖垮同进程所有请求，而 Go 限不住单个 goroutine 的 CPU；
> **三是权限**——同进程里能直接读宿主文件、发任意网络请求、执行任意 syscall；
> **四是逃逸**——Go 没有语言级安全边界，reflect/unsafe/cgo 都能突破。
>
> 所以要上进程级隔离，典型是容器：独立 rootfs + netns + PID ns + 非 root + seccomp + cgroup。不过容器共享宿主内核，内核漏洞就是逃逸路径，更严的场景用 gVisor 或 Firecracker/Kata 再包一层。
>
> 工程上的真实难点是冷启动——每请求起容器太慢，得靠预热池或 microVM 快照。"

### 追问预案

| 追问 | 怎么答 |
|---|---|
| Docker 沙箱怎么加固？ | 非 root + 只读 rootfs + drop cap + seccomp + 禁网 + cgroup + tmpfs |
| 容器逃逸怎么防？ | 隔离强度光谱：容器 → gVisor → microVM |
| 启动开销怎么控？ | 预热池 / 常驻沙箱复用 / Firecracker 快照恢复 |
| 沙箱能联网吗？ | 默认断网，需要就白名单——重点是**防 SSRF 打内网** |
| 超时怎么做？ | 强制 kill **容器**（不是 kill 进程），并防僵尸沙箱 |
| 跑个算法题也要 Docker 吗？ | 不一定。单进程无网络、输入输出明确的判题场景，`rlimit` + seccomp + 独立用户更轻更快；Docker 的价值在"要完整环境 + 强隔离" |

**关键提醒**：别把"panic 崩进程"当成主要理由——它是最轻的那类失败；**主菜是资源失控和权限逃逸**，那才是没有沙箱就真的没辙的部分。
