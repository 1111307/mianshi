...
- fieldrield 上有 `oldbuckets` 指针存旧表，全部搬完才释放。
...
type hchan struct {
    ...
    dataqsiz uint           // 环形队列容量（make(chan T, N) 的 N）
    buf      unsafe.Pointer // 指向环形队列缓冲区（无缓冲 chan 时为 nil）
    elemsize uint16         // 元素大小
    closed   uint32         // 是否已关闭
    elemtype *_type         // 元素类型
    sendx    uint           // 发送指针（环形队列下标）
    recvx    uint           // 接收指针
...
**要点**：chan 本质是一个**带锁的环形队列 + 两个 FIFO 等待队列**（sudog 链表，每个 sudog 包裹一个阻塞的 G）。

### 发送流程 `ch <- v`

1. 加锁；
2. 如果 `recvq` 有等待的接收者 → **直接把值拷贝**到接收 G 的栈上（绕过 buf，零拷贝到内存队列），唤醒该 G；
3. 否则如果 buf 有空位（`qcount < dataqsiz`）→ 拷贝进 buf 的 `sendx` 位置，`sendx = (sendx+1) % dataqsiz`；
4. 否则 → 当前 G 封装成 sudog 挂入 `sendq`，**gopark 挂起**，让出 M。
...
1. 加锁；
2. 如果 `sendq` 有等待的发送者：
   - 无缓冲 chan → 直接把发送 G 的值拷贝给接收方；
   - 有缓冲 chan → 先把 buf 头部元素取走给接收方，再把等待发送 G 的值塞进 buf 尾部（保持 FIFO），唤醒它；
3. 否则如果 buf 非空（`qcount > 0`）→ 从 `recvx` 位置取走；
4. 否则 → 挂入 `recvq`，gopark。

### close 发生了什么

- 置 `closed = 1`，一次性唤醒 `recvq + sendq` **所有**等待的 G；
- 对唤醒的接收者：返回**零值 + ok=false**；对唤醒的发送者：**panic("send on closed channel")**。
- 所以"向已关闭的 chan 发送会 panic，从已关闭的 chan 接收会返回零值和 false"是这条直接推论。

### 无缓冲 vs 有缓冲的本质差别

- **无缓冲**（`make(chan T)`）：buf 为 nil，发送和接收必须**同时在场**完成"手递手"，发送者和接收者**同步**进 rendezvous——所以 default 分支会立刻走，非阻塞收发必然失败。
- **有缓冲**：buf 是仓库，发送/接收解耦，一方可在另一方不在场时先写/先读。
