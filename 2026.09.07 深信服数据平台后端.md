...
3. **收发都是「先看对方队列」，不是「先看 buf」**（源码核对 `runtime/chan.go`）：

   ```
   发送 ch<-v：lock → closed?panic → 【先看 recvq】有人等 → 直接 send()（绕过 buf）
              → qcount < dataqsiz → 入仓 → 都不行才挂 sendq
   接收 <-ch：lock → closed且qcount==0 → 零值,false
              → 【先看 sendq】有人等 → recv()（无缓冲直取；有缓冲取头部+补尾部）
              → qcount > 0 → 从 recvx 取走 → 都不行才挂 recvq
   ```

   - 对 buffered chan，"有等待 sender" ⟹ 仓库必满，此时 sendq 与 qcount 同时成立，走 recv() 分支——它**取 buf 头部（recvx）的值、把 sender 的值补到尾部**，所以**交付给接收方的永远是队首值，FIFO 保序**。
   - 所以"保序"这个结论是对的，但"接收先看 buf"这个路径描述是错的——**代码里 sendq 在 qcount 之前检查**。
4. **判空/满用的是 `qcount`，不是比较两个索引**：环形队列下 `sendx == recvx` 有两种含义（全空 or 全满），有歧义。所以 `hchan` 额外维护了 `qcount`（当前元素数），空/满都看它。
...
### 从 `kubectl apply` 到流量进来：一条命令的接力（组件怎么协同）

![从 kubectl apply 到流量进来的九步接力](assets/k8s-02-sequence.png)

九步串起来（**每一步都是 watch 驱动，组件之间零直连**）：

```
① kubectl        apply -f deployment.yaml：声明"期望 3 副本"，不指定机器
② apiserver      认证→鉴权→准入→校验 → 对象写入 etcd（此时还没有任何 Pod）
③ Deployment 控制器  watch 到新 Deployment → 对账（期望 RS=1，实际 0）→ 创建 ReplicaSet
④ ReplicaSet 控制器  watch 到新 RS → 对账（期望 3 Pod，实际 0）→ 创建 3 个 Pod
                   此时 .spec.nodeName 为空（PENDING，未调度）
⑤ kube-scheduler  watch 到未调度 Pod → Filter → Score → 写回 nodeName（Binding）
                   ★ 只写一个字段，不碰容器
⑥ kubelet(Node)   watch 到分给本节点的 Pod → CRI 拉镜像 → sandbox → initContainers → 业务容器
⑦ kubelet         readiness 探针通过 → 回写 Pod.status=Running → apiserver → etcd
⑧ kube-proxy      watch 到 Endpoints 更新 → 刷本机 iptables/ipvs → Service VIP 有了真实后端
⑨ 流量进来        客户端访问 Service → 转发到 Pod → 链路打通
```

**关键认知：期望状态（spec）和实际状态（status）是两条路**——spec 由**命令面**（kubectl / 控制器）写入，status 由**节点面**（kubelet）回报。控制器做的事只有一件：**不断把 status 扳回 spec**。所以任何一步失败都不用管，下一轮对账会重试（这就是"声明式"：你说要什么，不说怎么做；扩容、滚动更新、自愈都变成同一件事——改期望值，让循环自己收敛）。
...
### 一张图记住组件协同：一切围着 apiserver 转

![K8s 组件协同：apiserver 是唯一枢纽](assets/k8s-01-arch.png)

```
                 kubectl / Client
                        │  HTTPS / List-Watch
                        ▼
                 kube-apiserver  ◄── 所有组件唯一的对话对象
                        │
                        ▼
                      etcd       ←── 唯一数据库，Raft 强一致，只有 apiserver 读写
                        ▲
        ┌───────────────┼───────────────┬──────────────────┐
        │               │               │                  │
...
1. **组件之间零直连**：scheduler 和 kubelet 从不直接通信，全靠 apiserver 中转——所以挂掉一个不阻塞别人。
2. **只有 apiserver 碰 etcd**：整个集群的读写入口唯一，权限、审计、准入都在这里收口。
3. **全部 watch 驱动 + 声明式对账**：没人"指挥"，每个组件只看自己关心的对象，变化就干活。

### 追问：为什么不用 Job 而用 Deployment？（工作负载选型）

**核心一句话**：Deployment 的期望终态是「**Pod 一直活着**」，Job 的期望终态是「**任务成功跑完**」——两个语义互斥。

**决定性硬约束在 `restartPolicy`**：

| | restartPolicy |
|---|---|
| **Job** | **只能是 `OnFailure` 或 `Never`**（写 `Always` 会被拒绝——Pod 退了就重启，任务永远结束不了） |
| **Deployment** | 必须 `Always`（默认），语义上要求 Pod 不退场 |

**用错的后果（两个方向都要能说）**：

```
方向一：用 Deployment 跑一次性任务 → 任务被【反复执行】
  脚本 exit 0 → ReplicaSet 只看"Pod 活不活着"，不看退出码
              → 发现副本数不足 → 创建新 Pod 再跑一遍 → 无限循环
  ★ 只要任务不幂等，就是重复扣款/重复发邮件/重复写数据
  ★ 而且 Deployment 没有"完成"状态，无法感知任务何时结束

方向二：用 Job 跑常驻服务 → 永远到不了 Complete
  → 失去"任务失败"这个信号（进程一直在跑，看起来正常）
  → 且 Job 重试次数有限（backoffLimit 默认 6），抖动几次就彻底放弃
```
...
| | Deployment | Job |
|---|---|---|
| 期望终态 | N 个 Pod 持续 Running | 任务**成功完成** |
| `restartPolicy` | `Always`（必须） | `OnFailure` / `Never` |
| Pod 正常退出 | 视为异常 → **重建** | 视为成功 → **不重建** |
| 完成状态 | 永远没有"完成" | `Complete` / `Failed` |
| 失败重试 | 无限（指数退避） | `backoffLimit`（默认 6）有限次 |
| 并发控制 | `replicas` | `parallelism` + `completions` |
| 超时控制 | 无（靠探针） | `activeDeadlineSeconds` |
| 自动清理 | 不需要 | `ttlSecondsAfterFinished` |
| **滚动更新/回滚** | ✅ 核心能力 | ❌ 无意义（一次性任务没有版本迭代） |
| **单例保证** | ❌ | ✅（`completions=1` 保证只成功一次） |
| 控制器层级 | Deployment → ReplicaSet → Pod | Job → Pod |

**Job 的几种模式**：

```
单次任务        completions=1, parallelism=1（默认）
固定完成数      completions=N          每个 Pod 处理一个工作项
工作队列模式    parallelism=M，不设 completions
              每个 Pod 从队列取任务直到队列空，然后自己退出
Indexed Job     1.24 GA：每个 Pod 拿唯一 index，适合分片处理
CronJob         定时任务 = Job + 调度表达式
```
...
| 场景 | 用什么 | 为什么 |
|---|---|---|
| Web 服务、API 网关 | **Deployment** | 应该一直活着 |
| 常驻消息消费者（Kafka consumer） | **Deployment** | 虽然"处理数据"但不退出，是长期服务 |
| **数据库 schema migration** | **Job** | 跑完就该退出，**绝不能重复执行**（最经典的坑） |
| 一次性数据迁移/批处理/离线计算 | **Job** | 有明确的"做完"时刻 |
| 定时对账/报表/清理 | **CronJob** | 周期性一次性任务 |

**两个容易混的相邻概念**：

- **Init Container** 也是一次性（跑完退出），但它是 **Pod 内部的启动阶段机制**，不跨 Pod，只负责"主容器启动前把前置条件做完"，**不是 Job**。
- **常驻消费者 vs 批处理消费者**：同一套业务逻辑，常驻的用 Deployment，一批数据跑完就退出的用 Job——形态决定控制器。
